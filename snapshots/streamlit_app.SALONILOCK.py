...
def build_signal_table(chart_df: pd.DataFrame) -> pd.DataFrame:
    table_df = chart_df.loc[:, ["DateLabel", "TimeLabel", "Close", "Signal"]].rename(
        columns={
            "DateLabel": "Date",
            "TimeLabel": "Time",
            "Close": "Price",
    ...
    table_df["Trade"] = table_df["Trade"].map({"BUY": "B", "SELL": "S"}).fillna(table_df["Trade"])
    return _format_table_dates(table_df)
        ...
        if isinstance(state, dict):
            ...
            try:
                ...
                try:
                    ...
                    rows = []
    st.session_state.saved_signals_selected_row = rows[0] if rows else None
...
def style_signal_table(table_df: pd.DataFrame) -> pd.io.formats.style.Styler:
    safe_df = _ensure_unique_columns(table_df)
    clicked_date = _scalarize(st.session_state.get("clicked_date"))
    clicked_time = _scalarize(st.session_state.get("clicked_time"))
    ...
    def style_rows(row: pd.Series):
        ...
        is_buy = trade_value == "B"
        color = BUY_DARK_COLOR if is_buy else SELL_DARK_COLOR
        trade_idx = list(row.index).index("Trade")
        price_idx = list(row.index).index("Price")
        styles[trade_idx] = f"color: {color}; font-weight:700;"
        styles[price_idx] = f"color: {color}; font-weight:700;"

        row_date = _scalarize(row.get("Date"))
        row_time = _scalarize(row.get("Time"))
        if (
            clicked_date is not None
            and clicked_time is not None
            and row_date == clicked_date
            and row_time == clicked_time
        ):
            styles = ["background-color: rgba(255,215,0,0.35);"] * len(row)
            styles[trade_idx] = f"color: {color}; font-weight:800;"
            styles[price_idx] = f"color: {color}; font-weight:800;"
...
def style_saved_signals_table(table_df: pd.DataFrame) -> pd.io.formats.style.Styler:
...
def main() -> None:
    ...
    st.session_state.setdefault("last_click_payload", None)
    ...
    st.session_state.setdefault("show_full_data", True)
    ...
    st.session_state.setdefault("right_panel_mode", "Saved Signals")
    ...
    if not data_dir.exists():
        st.error(f"Input Files folder not found in {main_dir}")
        return
    ...
    if not output_dir.exists():
        st.error(f"Output Files folder not found in {main_dir}")
        return
    ...
    default_from_date = max(min_date, max_date - pd.Timedelta(days=DEFAULT_RECENT_DAYS - 1))
    ...
    if st.session_state.show_filters:
        with st.sidebar:
            ...
            st.markdown("---")
            st.session_state.show_full_data = st.checkbox(
                "Show Full Data",
                value=st.session_state.show_full_data,
            )
            ...
            st.markdown("---")
            st.session_state.right_panel_mode = st.selectbox(
                "Right Panel",
                ["Saved Signals", "Signal Table", "None"],
                index=["Saved Signals", "Signal Table", "None"].index(st.session_state.right_panel_mode),
            )
    else:
        from_date = st.date_input(
            "From Date",
    ...
    if was_limited:
        shown_start = chart_df["Date"].dt.strftime("%d-%b-%Y").iloc[0]
        shown_end = chart_df["Date"].dt.strftime("%d-%b-%Y").iloc[-1]
        st.warning(
            f"Showing first {MAX_CANDLES:,} candles from {shown_start} to {shown_end} for smooth performance."
        )
    ...
    with col_center:
        ...
        btn_left, btn_mid, btn_right = st.columns([0.05, 2, 1.95], gap="small")
        with btn_left:
            st.empty()
        with btn_mid:
    ...
    if st.session_state.right_panel_mode != "None":
    ...
    if chart_event:
        ...
        st.session_state.last_click_payload = chart_event
    ...
    if table_col is not None and st.session_state.right_panel_mode == "Signal Table":
        with table_col:
            st.markdown("**Signal Table**")
            st.caption("Click a row to create BUY or SELL.")

            clicked_date = _scalarize(st.session_state.get("clicked_date"))

            # build table
            signal_table_df = build_signal_table(chart_df)

            # filter to clicked day
            clicked_epoch = _scalarize(st.session_state.get("clicked_epoch"))
            if clicked_date is not None and clicked_epoch is not None:

                # filter the clicked day
                day_df = chart_df[chart_df["DateLabel"] == clicked_date].reset_index(drop=True)

                # find nearest candle using epoch (exact match with chart)
                epoch = int(clicked_epoch)

                diff = (day_df["TimeEpoch"] - epoch).abs()
                nearest_idx = diff.idxmin()

                start = max(0, nearest_idx - 10)
                end = min(len(day_df), nearest_idx + 10)

                day_slice = day_df.iloc[start:end]

                signal_table_df = day_slice.rename(
                    columns={
                        "DateLabel": "Date",
                        "TimeLabel": "Time",
                        "Close": "Price",
                ...
                )[["Date", "Time", "Price", "Trade"]].reset_index(drop=True)
                signal_table_df["Trade"] = signal_table_df["Trade"].map({"BUY": "B", "SELL": "S"}).fillna(signal_table_df["Trade"])
                signal_table_df = _format_table_dates(signal_table_df)
        
            table_selection = st.dataframe(
                style_signal_table(signal_table_df),
                width="stretch",
                height=_table_height_for_rows(len(signal_table_df)),
                hide_index=True,
                on_select="rerun",
                selection_mode="single-row",
                key=f"signal-table-{symbol}",
                column_order=["Date", "Time", "Trade", "Price"],
            )
    selected_row = None

    if table_selection is not None:
        try:
            selected_rows = list(table_selection.selection.rows)
        except Exception:
            selected_rows = []
        ...
        if selected_rows:
            ...
            if 0 <= row_index < len(signal_table_df):

                row = signal_table_df.iloc[row_index]

                selected_row = chart_df[
                    (chart_df["DateLabel"] == row["Date"]) &
                    (chart_df["TimeLabel"] == row["Time"])
                ].iloc[0]

                signal_record = build_signal_record(symbol, selected_row)
                save_signal(signal_record, output_csv_path)

                # No manual rerun here to avoid chart reset loops.

    table_df = chart_df.loc[:, ["DateLabel", "TimeLabel", "Open", "High", "Low", "Close", "EMA"]].rename(
        columns={"DateLabel": "Date", "TimeLabel": "Time"}
    )

    tab_labels: list[str] = []
    if st.session_state.show_full_data:
        tab_labels.append("Full Data")
    if st.session_state.show_debug:
        tab_labels.append("Debug")

    if not tab_labels:
        st.caption("All tabs are hidden. Enable them from the sidebar.")
    else:
        ...
        if st.session_state.show_debug:
            with tabs[tab_idx]:
                if st.session_state.last_click_payload is None:
                    st.caption("No clicked candle payload yet.")
                else:
                    st.json(st.session_state.last_click_payload)

    # Saved Signals panel moved to the right column.
    if table_col is not None and st.session_state.right_panel_mode == "Saved Signals":
        with table_col:
            ...
            if st.session_state.saved_signals:
                ...
                saved_column_order = (
                    ...
                    else ["Date", "Time", "Trade", "Price"]
                )
                selection = st.dataframe(
                    style_saved_signals_table(saved_view),
                    width="stretch",
                ...
                selected_row = st.session_state.get("saved_signals_selected_row")
                selected_key = None
                if isinstance(selected_row, int) and 0 <= selected_row < len(saved_view):
                    selected_key = saved_view.iloc[selected_row]["SignalKey"]

                if st.button("Remove Selected", width="stretch", disabled=selected_key is None):
                    if selected_key is not None:
                        updated_signals = [
                            item for item in st.session_state.saved_signals
                            if item["SignalKey"] != selected_key
                        ]
                        try:
            ...
            if st.button("Clear Signals", width="stretch"):
                st.session_state.confirm_clear_all = True
            if st.session_state.confirm_clear_all:
                st.markdown(
                    "<div class='confirm-clear'>Clear all saved signals?</div>",
                    unsafe_allow_html=True,
                )
                if st.button("Confirm Clear All", width="stretch"):
                    try:
                        persisted_signals = persist_saved_signals_file(output_csv_path, symbol, [])
                    except Exception as exc:
                        st.error(f"Could not clear saved-signal file: {exc}")
                    else:
                        apply_saved_signals_state(persisted_signals, symbol, output_csv_path)
                        st.session_state.confirm_clear_all = False
                        st.rerun()
